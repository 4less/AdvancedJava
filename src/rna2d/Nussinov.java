...
public class Nussinov {
    ...
    public static int[][] pairArray(ArrayList<Pair<Integer,Integer>> pairArrayList) {
        int[][] result = new int[pairArrayList.size()][2];
        for (int i = 0; i < pairArrayList.size(); i++) {
            result[i][0] = pairArrayList.get(i).getKey();
            result[i][1] = pairArrayList.get(i).getValue();
        }
        return result;
    }
